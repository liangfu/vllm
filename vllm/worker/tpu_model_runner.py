...
class ModelWrapper(nn.Module):
    ...
    def forward(
        ...
        # add sort
        logits, indeces = torch.sort(input=logits, dim=-1, descending=True)
        ...
        # rand_selector = torch.rand((probs.shape[0], num_samples), device=probs.device)
        # probs_cumsum = torch.cumsum(input=probs, dim=-1)
        # greater_than_rand = torch.greater(rand_selector.unsqueeze(-1), probs_cumsum.unsqueeze(1))
        # sampled_token_ids = torch.sum(greater_than_rand, dim=-1)

        def _multinomial(probs, dim, num_samples=1):
        ...
        # , indeces = torch.sort(input=logits, dim=-1, descending=True)
        logits = torch.softmax(logits, dim=-1, dtype=torch.float32)

        counts = _multinomial(logits, -1)
        output_indices = torch.gather(input=indeces, dim=-1, index=counts).flatten().unsqueeze(-1)
        for _ in range(num_samples-1):
            counts = _multinomial(logits, -1)
            output_indice = torch.gather(input=indeces, dim=-1, index=counts).flatten()
            output_indices= torch.cat((output_indices, output_indice.unsqueeze(-1)), dim=-1)
        
        sampled_token_ids = output_indices
