...
def ref_masked_attention(
    ...
    print(f"scaled ref_mask_attention q {scale*query}")
    print(f"ref_mask_attention key {key}")
    print(f"ref_mask_attention value {value}")
    print(f"sclae is {scale}")
    print(f"ref_mask_attention scaled_qk {scaled_qk}")
    ...
    print(f"norms_score {norm_score}")
...
def test_contexted_kv_attention(
    ...
    output_ref = output_nki.transpose(0, 1)[0, :num_tokens, :, :]
...
def test_context_attention_fwd(
    ...
    v_cache = torch.zeros(cache_size,
                          ...
                          head_size,
                          dtype=dtype)
    k = torch.zeros(num_tokens, num_kv_heads, head_size, dtype=dtype)
    v = torch.zeros(num_tokens, num_kv_heads, head_size, dtype=dtype)
    values = torch.arange(0, cache_size, dtype=torch.long)
    values = values[torch.randperm(cache_size)]
    block_table = values[:batch_size * max_block_per_request].view(
        batch_size, max_block_per_request)
    ...
    pad_kv_dims = (
        ...
        0,
        0,
        max_num_queries_padded - sum(seq_lens),
    )
    # padding for neuron device
    ...
    print(f"original prior mask is {prior_mask}")
    print(f"original active mask is {active_mask}")
    ...
    # print(f"attn prior mask is {attn_mask[0]}")
    # print(f"attn active mask is {attn_mask[1]}")
    ...
    if return_debug_tensors:
        output_nki, *debug_tensors = context_attention_fwd(*example,
                                                           simulate=simulate, return_debug_tensors=return_debug_tensors)
        # print(f"debug tensor hbm_m_buffer {debug_tensors[0]}")
        # print(f"debug tensor hbm_l_buffer {debug_tensors[1]}")
        # print(f"debug tensor hbm_qk_res {debug_tensors[2]}")
    else:
        output_nki = context_attention_fwd(*example, simulate=simulate, return_debug_tensors=return_debug_tensors)
        debug_tensors = []
    ...
    output_ref = output_ref.squeeze(1)
    print(f"output_ref is {output_ref}")
    print(f"output_nki is {output_nki}")
    torch.testing.assert_close(output_nki, output_ref, atol=1e-2, rtol=0)
...
head_size = 3
output = test_context_attention_fwd(num_heads, num_queries_per_kv, head_size)
