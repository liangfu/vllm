...
def sample_inputs(
    ...
    max_model_len = (max_query_len + max_ctx_len) * 16
    assert max_model_len >= block_size, \
        "{max_model_len=} is expected to be greater than {block_size=}"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # (1, 7, 1, 512, 4, 2, 8, False),  # minimal block size, small dimensions
        # (1, 7, 1, 512, 4, 2, 8, True),  # same with mixed precision
        # 
        ...
        # Test edge cases
        (1, 127, 16, 1024, 4, 2, 16, False),  # large decode batch
...
def test_contexted_kv_attention(
    ...
    ctx_lens_blockaligned = ((torch.tensor(ctx_lens) + block_size - 1) // block_size) * block_size
    cu_ctx_lens = torch.tensor([0] + ctx_lens_blockaligned.tolist(),
                               dtype=torch.int32).cumsum(dim=0,
                                                         dtype=torch.int32)
    ...
    output_nki, prior_mask_output, active_mask_output = flash_attn_varlen_func(*input_args, **input_kwargs)

    import numpy as np
    prior_mask_output = prior_mask_output.astype(np.int32)
    active_mask_output = active_mask_output.astype(np.int32)
    ...
    # prior_mask_output = prior_mask_output.cpu().numpy()
    # active_mask_output = active_mask_output.cpu().numpy()
    # torch.testing.assert_close(attn_mask.int()[:128,:1024], torch.tensor(prior_mask_output).int()[0,0,:,:1024])
